...
$sql = "SELECT * FROM daily WHERE code = '$code_request' AND daily_status != 'รอการยืนยัน'";
    ...
    <div class="letter">
        <div style="padding-bottom:32px;">
            ...
            <table>
                ...
                <tr>
                    <td>1</td>
                    <td colspan="3">ห้องแอร์</td>
                    <td><?php echo $air_room; ?></td>
                    <td><?php echo number_format($air_price - (($air_price * 7)/100),2); ?></td>
                </tr>
                <?php } ?>
                <?php
                if($fan_room != 0){
                ?>
                <tr>
                    <td><?php if($air_room != 0){ echo 2; }else{ echo 1; }?></td>
                    <td colspan="3">ห้องพัดลม</td>
                    <td><?php echo $fan_room; ?></td>
                    <td><?php echo number_format($fan_price - (($fan_price * 7)/100),2); ?></td>
                </tr>
                <?php } ?>
                <tr>
                    <td colspan="4"><?php echo bathformat($total_price) ?></td>
                    <td style="text-align:center;">ภาษีมูลค่าเพิ่ม 7%</td>
                    <td><?php echo number_format(($total_price * 7) / 100,2); ?></td>
                </tr>
                <tr>
                    <td colspan="4"></td>
                    <td style="text-align:center;">จำนวนเงินรวมทั้งสิ้น</td>
                    <td><?php echo number_format(ceil($total_price),2); ?></td>
                </tr>
            </table>
